...
export default function Stores() {
    ...
    async function handleStoreProducts(event: any) {
        setLoading(true);
        event.preventDefault();
        const [urlSearchStore, stores] = event.target.value.split('|');
        const valorCodificado = encodeURIComponent(String(urlSearchStore));
        const apiClient = setupAPIClient();
        try {
            await apiClient.get(`/search_products?urlSearchStore=${valorCodificado}&stores=${stores}`);
            setLoading(false);
